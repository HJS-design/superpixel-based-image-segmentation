...
int numVariables;
	...
	std::cout << "read_mat:"<<(des / "testdata.mat").string();
#ifdef debug_flag
	std::filesystem::create_directories(log_file.debug_file);
#endif // debug_flag
	 matGetDir(pmat, &numVariables);
	 matClose(pmat);

	 
	 pmat = matOpen((des / "testdata.mat").string().c_str(), "r");
...
prhs[numVariables] = mxCreateString(des.string().c_str());
#endif // output_label_each_step
	
	mxArray* plhs[2];

#ifdef output_label_each_step
mexFunction(2, plhs, numVariables+1, prhs);
#else
mexFunction(2, plhs, numVariables, prhs);
#endif // output_label_each_step

	

	pmat = matOpen((des / "output.mat").string().c_str(), "w");
	std::cout << "save_mat:" << (des / "output.mat").string();
	matPutVariable(pmat, "region", plhs[0]);
	matPutVariable(pmat, "recorded_edge", plhs[1]);

	//matPutVariable(pmat, "homo", plhs[1]);
